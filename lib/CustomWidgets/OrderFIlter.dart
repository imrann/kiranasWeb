...
class _OrderFilterState extends State<OrderFilter> {
      ...
      if (isShown) {
        if (widget.orderType.contains("Delivered")) {
          ...
          if (deliveredOrderdFilterState.getIsClearFilter()) {
            OrderController().getOrdersByType("Delivered").then((value) {
              var ordersListState =
          ...
          if (cancelledOrderdFilterState.getIsClearFilter()) {
            OrderController().getOrdersByType("Cancelled").then((value) {
              var ordersListState =
                  Provider.of<OrdersListState>(context, listen: false);
          ...
          if (openOrderdFilterState.getIsClearFilter()) {
            OrderController().getOrdersByType("Open").then((value) {
              var ordersListState =
                  Provider.of<OrdersListState>(context, listen: false);
  ...
  showDate() {
    showDatePicker(
      ...
      // var nextDate = date.add(Duration(days: 1));
      // var formattedNextDate = format.format(nextDate);
      // var epochConvertedNextDate = nextDate.millisecondsSinceEpoch;
      // print(formattedNextDate.toString() +
      //     "  " +
      //     epochConvertedNextDate.toString());

      // print(date1.toString());
      // print(newAddedDate.toString());
